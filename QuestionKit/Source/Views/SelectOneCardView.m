...
- (id) initWithPrompt:(NSDictionary *) prompt changeAction:(void (^)(NSString * key, id value)) changeAction {
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        self.checkGroup = [BEMCheckBoxGroup groupWithCheckBoxes:self.checkBoxes];
    }
    ...
    for (NSUInteger i = 0; i < self.checkBoxLabels.count; i++) {
        ...
        if (self.delegate != nil) {
            if ([self.delegate respondsToSelector:@selector(fillColor)]) {
                checkBox.onFillColor = [((id<SelectOneCardViewDelegate>) self.delegate) fillColor];
            }

            if ([self.delegate respondsToSelector:@selector(tintColor)]) {
                checkBox.onTintColor = [((id<SelectOneCardViewDelegate>) self.delegate) tintColor];
                checkBox.tintColor = [((id<SelectOneCardViewDelegate>) self.delegate) tintColor];
            }

            if ([self.delegate respondsToSelector:@selector(checkColor)]) {
                checkBox.onCheckColor = [((id<SelectOneCardViewDelegate>) self.delegate) checkColor];
            }
        }
